...
def create_app():
    ...
    # Uprawnienia plików SSL
    ssl_perms = security.secure_ssl_file_permissions()
    if ssl_perms:
        app_logger.info("Uprawnienia plików SSL zabezpieczone")
    else:
    ...
    if not admin_password_hash:
        app_logger.critical("BŁĄD KRYTYCZNY: ADMIN_PASSWORD_HASH nie został ustawiony w .env!")
        app_logger.critical("Aplikacja nie może działać bez bezpiecznego hasła administratora.")
        app_logger.critical("Ustaw ADMIN_PASSWORD_HASH w .env i uruchom aplikację ponownie.")
        raise RuntimeError(
            "KRYTYCZNY BŁĄD BEZPIECZEŃSTWA: Brak hasła administratora w konfiguracji. "
            "Ustaw ADMIN_PASSWORD_HASH w .env przed uruchomieniem aplikacji."
        )
...
def configure_flask_settings(app):
    """Konfiguruje ustawienia Flask."""
    from flask import request, redirect
    
    app.secret_key = os.getenv('FLASK_SECRET_KEY')
    if not app.secret_key:
        logger = logging.getLogger(__name__)
        logger.warning("FLASK_SECRET_KEY nie ustawiony. Generuję losowy klucz.")
        app.secret_key = secrets.token_hex(32)
        logger.info("Wygenerowano losowy klucz sesji.")
    
    app.config['MAX_CONTENT_LENGTH'] = 16 * 1024 * 1024  # 16MB
    
    # JSON dla polskich znaków
    ...
    def force_https():
        ...
        # Za HTTPS proxy nie przekierowuj
...
def configure_ssl_production():
    """Konfiguruje SSL dla produkcji."""
    # Sprawdź tryb aplikacji
    flask_env = os.getenv('FLASK_ENV', 'development').lower()
    
    if flask_env == 'development':
        # W trybie development nie używamy SSL
        logger = logging.getLogger(__name__)
        logger.info("Tryb development - SSL wyłączony")
        return None, 5001
